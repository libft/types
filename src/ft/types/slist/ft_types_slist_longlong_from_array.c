...
{
	t_ft_types_slist_longlong	result;
	size_t						i;

	ft_types_slist_longlong_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_longlong_push(&result, array->element[i]))
		{
			ft_types_slist_longlong_clear(&result);
			return (true);
		}
	}
	*out = result;
