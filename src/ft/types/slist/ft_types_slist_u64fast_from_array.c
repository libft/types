...
{
	t_ft_types_slist_u64fast	result;
	size_t						i;

	ft_types_slist_u64fast_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_u64fast_push(&result, array->element[i]))
		{
			ft_types_slist_u64fast_clear(&result);
			return (true);
		}
	}
	*out = result;
