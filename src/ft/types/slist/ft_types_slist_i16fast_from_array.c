...
{
	t_ft_types_slist_i16fast	result;
	size_t						i;

	ft_types_slist_i16fast_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_i16fast_push(&result, array->element[i]))
		{
			ft_types_slist_i16fast_clear(&result);
			return (true);
		}
	}
	*out = result;
