...
{
	t_ft_types_slist_i64least	result;
	size_t						i;

	ft_types_slist_i64least_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_i64least_push(&result, array->element[i]))
		{
			ft_types_slist_i64least_clear(&result);
			return (true);
		}
	}
	*out = result;
