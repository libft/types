...
{
	t_ft_types_slist_ptrdiff	result;
	size_t						i;

	ft_types_slist_ptrdiff_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_ptrdiff_push(&result, array->element[i]))
		{
			ft_types_slist_ptrdiff_clear(&result);
			return (true);
		}
	}
	*out = result;
