...
{
	t_ft_types_slist_func_pu8l_pu8l_p	result;
	size_t								i;

	ft_types_slist_func_pu8l_pu8l_p_init(&result);
	i = -1;
	while (++i < array->count)
	{
		if (ft_types_slist_func_pu8l_pu8l_p_push(&result, array->element[i]))
		{
			ft_types_slist_func_pu8l_pu8l_p_clear(&result);
			return (true);
		}
	}
	*out = result;
